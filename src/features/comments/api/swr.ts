...
export function useComments(postId?: string) {
  ...
  return useSWR<Comment[]>(key, () => fetchComments(postId!), {
    ...
    await mutate(
      ...
      async () => {
        ...
        await mutate(
          ...
          (c?: number) => (typeof c === "number" ? c + 1 : 1),
          false
        );
        return fetchComments(postId);
      },
      {
        optimisticData: (current?: Comment[]) => [
          ...(current ?? []),
          optimistic,
        ],
...
export function useCommentsCount(postId?: string) {
  return useSWR<number>(postId ? `/posts/${postId}/comments/count` : null, () =>
